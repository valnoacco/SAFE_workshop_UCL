...
def relative_error(y_sim, y_obs):

    """Computes the relative error (% change between observed and predicted)

    rel_err = relative_error(Y_sim,y_obs)
    ...
    rel_err   = vector of relative errors           - vector (N,1)
    ...
    rel_err = np.mean((y_sim - repmat(y_obs, N, 1)/repmat(y_obs, N, 1)), axis=1)*100
